...
//
    @Query("select e from Employee e inner join e.address a where e.department=?1 and a.city=?2")
    List<Employee> findByDeptCity(String dept, String city);
//
    @Query("select e from Employee e inner join e.address a where e.salary < ?1 and a.city = ?2")
